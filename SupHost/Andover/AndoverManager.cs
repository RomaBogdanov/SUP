...
namespace SupHost.Andover
{
	public class AndoverManager
	{
		...
		private bool UpdateDoors(List<Door> doors, List<Area> areas, List<Container> containers)
		{
			...
			{
				int spaceInIdHi = door.EntryAreaHi.HasValue ? door.EntryAreaHi.Value : 0;
				int spaceInIdLo = door.EntryAreaLo.HasValue ? door.EntryAreaLo.Value : 0;

				int spaceOutIdHi = door.ExitAreaHi.HasValue ? door.ExitAreaHi.Value : 0;
				int spaceOutIdLo = door.ExitAreaLo.HasValue ? door.ExitAreaLo.Value : 0;
		...
		private bool UpdateSchedules(List<Schedule> schedules, List<Container> containers)
		{
			...
			foreach (var schedule in schedules)
			{
				...
				row["f_schedule_path"] = schedule.Path;
